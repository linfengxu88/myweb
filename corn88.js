...
(function () {
  ...
  const findInputById = (ids) => {
    const idList = Array.isArray(ids) ? ids : [ids];
    return idList.map((id) => document.getElementById(id)).find(Boolean);
  };
    ...
    try {
      await Promise.all(
        ["name", "phone", "idNumber", "date"].map(async (key) => {
          const el = await waitFor(() => findInputById(FIELD_IDS[key]));
          fire(el, config[key]);
        })
      );
      log("基本信息已填入。");

      const checkbox = await waitFor(() => document.getElementById("txtS"));
      if (checkbox) {
        checkbox.click();
        log("已选择上午。");
      }
  ...
  async function selectCascade() {
    const selects = [
      { elementId: "province", value: config.province },
      { elementId: "city", value: config.city },
      { elementId: "area", value: config.county },
    ];

    const pickedLabels = [];
    for (const { elementId, value } of selects) {
      const option = await selectByValue(elementId, value);
      pickedLabels.push((option.textContent || option.value || "").trim());
    }
    log(`已选择 ${pickedLabels.join(" / ")}`);
  }

  async function selectByValue(selectId, targetValue) {
    if (!targetValue) throw new Error(`未配置 ${selectId} 下拉值`);
    const select = await waitFor(() => document.getElementById(selectId));
    const option = await waitFor(
      () => findSelectOption(select, targetValue),
      10000,
      100
    );
    option.selected = true;
    fire(select, option.value);
    return option;
  }

  const findSelectOption = (select, targetValue) => {
    const normalized = `${targetValue}`.trim();
    if (!normalized) return null;
    return Array.from(select.options).find((opt) => {
      const optValue = (opt.value || "").trim();
      const optText = (opt.textContent || "").trim();
      return (optValue && optValue === normalized) || optText === normalized;
    });
  };
